...
export default function AstePage() {
  ...
  const sourceColor = (name: string) =>
    SOURCES.find((s) => s.label === name)?.color || '#888'
    ...
    <div className="min-h-screen" style={{ background: '#05050B' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-28 pb-16">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">Scraping automatico dei lotti di mobili di design dalle principali case d&apos;asta italiane. Salva direttamente su Notion.</p>
        </div>

        {/* CONTROLS */}
        <div className="rounded-2xl p-6 mb-6" style={{ background: '#0D0D1A', border: '1px solid #1E1E30' }}>
          <p className="text-sm font-medium text-gray-400 mb-4">Case d&apos;asta da scrapare:</p>
          <div className="flex flex-wrap gap-3 mb-6">
            {SOURCES.map((s) => (
              <button
                ...
                style={{
                  background: selected.includes(s.key) ? `${s.color}20` : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${selected.includes(s.key) ? s.color : '#2A2A3A'}`,
                  color: selected.includes(s.key) ? s.color : '#666',
                }}
              >
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <button
              ...
              disabled={scraping || !selected.length}
            ...
            >
              {scraping ? (
                <>
                  <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="30 70"/></svg>
                  Scraping in corso…
                </>
              ) : (
                <>🔍 Avvia Scraping</>
            ...
            {lots.length > 0 && (
              <button
                onClick={handleSave}
                disabled={saving || !selectedLots.size}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all disabled:opacity-50"
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #1E1E30' }}>
            <div className="flex items-center justify-between px-5 py-4" style={{ background: '#0D0D1A', borderBottom: '1px solid #1E1E30' }}>
              <div className="flex items-center gap-3">
                <input type="checkbox" checked={selectedLots.size === lots.length} onChange={toggleAll}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div>
                        ...
                        {lot.numeroLotto && (
                          <span className="text-xs text-gray-500">Lotto {lot.numeroLotto}</span>
                        )}
                      </div>
                      <div className="text-right flex-shrink-0">
                        {(lot.stimaMin || lot.stimaMax) && (
                          <p className="text-sm font-semibold text-white">
                            {formatEuro(lot.stimaMin)} – {formatEuro(lot.stimaMax)}
                          </p>
                        )}
                        {lot.dataAsta && (
                    ...
                    {lot.descrizione && (
                      <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{lot.descrizione}</p>
                    )}
                    {lot.categoria && (
                      <span className="inline-block mt-1.5 text-xs px-2 py-0.5 rounded" style={{ background: '#1E1E30', color: '#888' }}>
                        {lot.categoria}
                      </span>
                    )}
                  </div>
        ...
        {!scraping && lots.length === 0 && !scrapeError && (
          <div className="text-center py-24" style={{ color: '#333' }}>
            <div className="text-6xl mb-4">🪑</div>
            <p className="text-gray-600">Seleziona le case d&apos;asta e avvia lo scraping</p>
          </div>
        )}
      </div>
    </div>
